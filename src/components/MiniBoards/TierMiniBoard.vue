<template>
  <div>
    <div class="d-flex mx-2" v-for="tier in board.lists" style="margin-bottom: 4px;" :key="tier.id">
      <div
        :style="`background-color: ${tier.backgroundColor}; border-radius: 4px !important; width: 20px; height: 20px;`" />

      <img v-for="(game, index) in tier.games" :key="index"
        :ref="el => { if (el) gameRefs[`tier-${tier.id}-game-${index}`] = el }" :class="[
        ...
        ]" :title="game.name" :src="showGameThumbnails && game.src ? game.src : null" :alt="game.name"
