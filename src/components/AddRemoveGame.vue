<template lang="html">
  <AppSidebar id="addRemoveGameSidebar" :visible="visible" :placement="sidebarRightProps?.placement || 'end'"
    :bg-variant="sidebarRightProps?.bgVariant" :text-variant="sidebarRightProps?.textVariant"
    @update:visible="handleVisibilityChange">
    <template #header>
      <SidebarHeader :title="`Add or remove ${game?.name || 'game'} to list`" @hide="hideSidebar" />
    </template>

    <div class="px-3 pb-3">
      <ul class="list-group">
        <li v-for="board in boards" :key="board.id" class="list-group-item"
          ...
          <div class="cursor-pointer d-flex align-items-center"
            ...
            <MiniBoard class="cursor-pointer" :board="board" thumbnail />

            <h3 class="mx-2 mb-0">{{ board.name }}</h3>
          </div>

          <div v-show="selectedBoard === board.id" class="collapse mt-2"
            :class="{ 'show': selectedBoard === board.id }">
            <ul class="list-group">
              <li v-for="(list, listIndex) in board.lists" :key="listIndex"
                class="list-group-item d-flex justify-content-between align-items-center"
                :class="darkTheme ? 'bg-dark text-light' : 'bg-light'">
                {{ list.name || '[Unnamed]' }}

                <button type="button" class="btn btn-sm" :class="isGameInList({ list }) ? 'btn-danger' : 'btn-success'"
                  @click="handleClick({ list, listIndex, board })">
                  {{ isGameInList({ list }) ? 'Remove' : 'Add' }}
                  <i :class="`fa fa-${isGameInList({ list }) ? 'trash' : 'plus'}`" aria-hidden="true" />
                </button>
              </li>
            </ul>
          </div>
